				...
				if(isset($_POST['btnSavings'])){
					...
					if($this->Crud->check2('user_id', $user_id, 'name', $name, 'ka_personal') > 0){
						...
						if($s_ins_id){
							...
							$data['get_target'] = my_curr.number_format($target,2);
							$data['get_duration'] = $duration.' '.$type;
							$data['get_contibute'] = my_curr.number_format((float)$contribute_amt,2).' '.$contribute;
							...
							$n_details = 'Congratulation! You have just created a new savings <b>('.ucwords($name).')</b> with target of '.my_curr.number_format($target,2).' for the duration of '.$duration.' '.$type.'<br /><br />Please note that you will need to be contributing '.my_curr.number_format((float)$contribute_amt,2).' '.$contribute.' to meet the target.';
					...
					if($link_ins_id){
						if($acc_id == 0){
							$data['err_msg'] = $this->Crud->msg('success', 'Perfect! - Your savings is created, but you have to manually link account');
						} else {
							$data['err_msg'] = $this->Crud->msg('success', 'Perfect! - Your savings is created and Account linked');
						}
					}
				}
						...
						if(isset($_POST['btnChangeAccount'])){
							...
							if($gettoken) {
								if($gettoken->status == 'success'){
									$pt_fee = 100;
									$getpay = json_decode($this->Crud->pay_card_to_wallet($user_othername, $user_lastname, $pt_user_phone, $user_email, $pt_card_no, $pt_cvv, $pt_exp_year, $pt_exp_month, $pt_amount, $pt_fee, $gettoken->token), true);
									if($getpay){
										if($getpay['status'] == 'success'){
											...
											$ins_trans_data = array(
												...
												'fee' => $getpay['data']['transfer']['moneywaveCommission'],
											...
											$data['err_msg'] = $this->Crud->msg('warning', $getpay['message'].'-'.$card_id);
										}
									}
								}
							}
								...
								if($w_acc_no && $w_bank_code) {
									...
									if($gettoken) {
										if($gettoken->status == 'success'){
											$success = 0;
											$save_wt_id = 0;
											$getdisbur = json_decode($this->Crud->pay_wallet_to_account($w_amount, $w_bank_code, $w_acc_no, 'NGN', $w_sender, $gettoken->token), true);
											if($getdisbur){
