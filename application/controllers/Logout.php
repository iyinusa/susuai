	...
	public function index() {
		...
		if($fb_access_token) {
		 ...
		 if(!empty($fbuser_info)) {
			  ...
			  $check_fb = $this->Crud->read_single('email', $fbuser_email, 'ka_user');
			  if(count($check_fb) <= 0){
				  // register user
				  ...
				  $fb_pass = md5($reg_date);
					...
					if($ins_id > 0) {
						...
						if($email_result == TRUE){
							// now admins
							$admin_list = 'iyinusa@yahoo.co.uk';
...
/////////////////////////////////////////// FB LOGIN ENDS //////////////////////////////////////////////
		...
		$kas_id = $this->session->userdata('kas_id');
		$data['reset'] = FALSE;
		
		$status_update = array('status'=>0);
		$this->Crud->update('id', $kas_id, 'ka_user', $status_update);
		$newdata = array(
			'kas_id' => '',
			'kas_user_email' => '',
			'kas_user_lastlog' => '',
			...
			'kas_user_state' => '',
			'kas_user_country_id' => '',
			'kas_user_country_name' => '',
			'kas_user_country_code' => '',
			'kas_user_country_currency' => '',
			'kas_user_marital' => '',
			'kas_user_pics' => '',
			'kas_user_bio' => '',
			'kas_user_role' => '',
			'kas_user_activation_code' => '',
			'kas_user_activate' => '',
			'kas_user_reg_date' => '',
			'logged_in' => FALSE
		);
